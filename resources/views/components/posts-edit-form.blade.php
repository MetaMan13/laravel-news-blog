<div class="h-5/6 w-4/12 mt-28 mx-auto">
    ...
    <div class="h-5/6 mt-10">
        <form action="/post/{{ $postId }}" class="w-full h-full" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mt-6 w-10/12 mx-auto text-center">
                <button type="submit" class="w-full bg-green-400 text-white font-semibold text-xl uppercase p-2 transition duration-250 ease-in-out hover:bg-green-600">Update post</button>
            </div>
        </form>
    </div>
</div>